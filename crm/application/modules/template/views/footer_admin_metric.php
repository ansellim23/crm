...
<script type="text/javascript">
 var pageURL = $(location).attr("href");
  ...
  if (pageURL == 'https://hlmcrm.site/dashboard/schedule_attendance'){
    ...
    <script src="<?php echo base_url('js/croppie.js');?>"></script>
     ...
     <script src="<?php echo base_url('js/validateattendancemetric.js');?>"></script>
 ...
 <script>  
    $('#agent_multiple').selectpicker('val', ['68', '70', '65']);
   ...
   function hmsToSeconds(s) {
    ...
    function CalculateTimeAverage(test_table) {
        ...
        var secondsToHms = function(d) {
          ...
          var intVal_call_log = function (i) {
            return typeof i === 'string' ?
                    i.replace(/[\$,]/g, '') * 1 :
                    typeof i === 'number' ?
                        i : 0;
       ...
       var numRows = api.rows( {search:'applied'} ).count();
        ...
        api.columns(".average_time").each(function (index) {
         ...
         var sum_hours_estimated = column_point 
            ...
            var  number_of_days = $('#call_logs_form [name="from_date"]').val() == "" ? 0 : count_date;

               $('.total_time_average_call_logs').text(sum_hours_estimated);
                  if(numRows =="" || numRows ==0){
                     $('.total_time_average_call_logs').text('00:00:00');
                     $('.total_handling_time').text('00:00:00');

                 }
                 else{
                    $('.total_time_average_call_logs').text(sum_hours_estimated);
                     $('.total_handling_time').text(secondsToHMS(hmsToSeconds(sum_hours_estimated)/numRows));
                 }

            
      });

         api.columns(".sum_of_call_log").each(function (index) {
         var column = api.column(index,{page:'current'});

           var sum = column 
                .data()
                .reduce( function (a, b) {
                    return intVal_call_log(a) + intVal_call_log(b);
                }, 0 );



             $('.total_call_logs').text(sum);
             if(sum =="" || sum ==0){
                $('.total_call_count').text('0');

             }
             else{
               $('.total_call_count').text(sum/numRows);
             }
      });
...
// JS for call logs
        $('#call_log_reportrange').on('apply.daterangepicker', function(ev, picker) {
                 ...
                 var get_agent_name = $('#call_logs_form .agent_name option:selected').text();

                 var agent_name = get_agent_name.split('-')[0];
             ...
             test_table =   $('#call_log_historytable').DataTable( {
                        ...
                        "columnDefs": [
                      {
                        ...
                        "ajax": {
                            ...
                            "url":   base_url +  "dashboard/export_call_log",
                            ...
                            "dataSrc":"",
                             ...
                             "data": { 
                                     ...
                                     "start": start,"end": end, "agent_name" : agent_name  
       ...
       test_table.on( 'draw', function () {
        ...
        //     $('.total_work_call').text(numRows.toLocaleString("en")+'.00');
...
$('#call_logs_form [name="user_type"]').on('change', function () {
                ...
                var get_agent_name = $('option:selected', this).text();
                var agent_name = get_agent_name.split('-')[0];

               var start =  $('#call_logs_form [name="from_date"]').val();
                var end =   $('#call_logs_form [name="to_date"]').val();
                 test_table =   $('#call_log_historytable').DataTable( {
...
$(function () {
 ...
 // checklead admin
      ...
      var table = $('#leaddataTablefixed_admin').DataTable({
      bInfo:      false,
      scrollY:        700,
      scrollCollapse: true,
      scroller:       true,
