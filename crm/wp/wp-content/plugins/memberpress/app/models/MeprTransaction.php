  ...
  public function store($keep_expires_at_time = false) {
    ...
    if(isset($this->id) && !is_null($this->id) && (int)$this->id > 0) {
      $this->id = self::update($this);
        ...
        if($user->roles[0] == 'subscriber' && $_POST['user_email'] == $user->user_email ){
            $wpdb->query($wpdb->prepare("DELETE FROM wpju_mepr_transactions WHERE product_id = '".$_POST['mepr_product_id']."' and status ='pending' order by id desc limit 1"));
        }
