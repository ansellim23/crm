...
class MeprAppHelper {
  ...
  public static function format_price_string($obj, $price = 0.00, $show_symbol = true, $coupon_code = null, $show_prorated = true, &$payment_required = true) {
    ...
    $user = MeprUtils::get_currentuserinfo($_GET['user']);
      ...
      if( $obj->trial ) {
        ...
        if( ( $obj instanceof MeprSubscription and $obj->prorated_trial ) or
          ...
          if( $obj instanceof MeprProduct ) {
            $usr = MeprUtils::get_currentuserinfo($_GET['user']);
            $grp = $obj->group();
  ...
  public static function countries_dropdown($field_key, $value=null, $classes='', $required_attr='', $geolocate=true, $unique_suffix='') {
    ...
    ob_start();
    ?>
    <select name="<?php echo $field_key; ?>" id="<?php echo $field_key . $unique_suffix; ?>" class="<?php echo $classes; ?> mepr-countries-dropdown mepr-form-input mepr-select-field" <?php echo $required_attr; ?>>
  ...
  public static function states_dropdown($field_key, $value=null, $classes='', $required_attr='', $geolocate=true, $unique_suffix='') {
    ...
    ob_start();
    ?>
    <select name="" data-fieldname="<?php echo $field_key; ?>" data-value="<?php echo esc_attr($value); ?>" id="<?php echo $field_key . $unique_suffix; ?>" class="<?php echo $classes; ?> mepr-hidden mepr-states-dropdown mepr-form-input mepr-select-field" style="display: none;" <?php echo $required_attr; ?>>
      <?php /* The js populates this part */ ?>
    </select>
    <?php /* Make sure the text box isn't hidden ... at the very least we need to see something! */ ?>
    <input type="text" name="<?php echo $field_key; ?>" data-fieldname="<?php echo $field_key; ?>" data-value="<?php echo esc_attr($value); ?>" id="<?php echo $field_key . $unique_suffix; ?>" class="<?php echo $classes; ?> mepr-states-text mepr-form-input" value="<?php echo esc_attr($value); ?>" <?php echo $required_attr; ?>/>
    <?php
    return ob_get_clean();
  }
