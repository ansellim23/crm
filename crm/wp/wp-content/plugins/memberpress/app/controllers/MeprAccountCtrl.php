...
class MeprAccountCtrl extends MeprBaseCtrl {
  ...
  public function maybe_update_username() {
    if( MeprUtils::is_post_request() &&
      ...
      $mepr_user = MeprUtils::get_currentuserinfo($_GET['user']);
  ...
  public function render() {
    ...
    $mepr_current_user = MeprUtils::get_currentuserinfo($_GET['user']);
  ...
  public function home() {
    ...
    $mepr_current_user = new MeprUser($mepr_current_user->ID);
       ...
       MeprView::render('/account/books', get_defined_vars());
  }
  ...
  public function save_new_password($user_id, $new_pass, $new_pass_confirm) {
    ...
    $user = MeprUtils::get_currentuserinfo($_POST['user']);
    ...
    if($mepr_options->enforce_strong_password && isset($_POST['mp-pass-strength']) && (int)$_POST['mp-pass-strength'] < MeprZxcvbnCtrl::get_required_int()) {
      MeprUtils::wp_redirect($account_url.$delim.'action=newpassword&user='.$user->ID.'&error=weak');
    }
    ...
    if($user_id && $user && ($user->ID==$user_id)) {
      if(($new_pass == $new_pass_confirm) && !empty($new_pass)) {
        ...
        $user->store();
        MeprUtils::wp_redirect($account_url.$delim.'action=home&user='.$user->ID.'&message=password_updated');
      }
    }

    MeprUtils::wp_redirect($account_url.$delim.'action=newpassword&user='.$user_id.'&error=failed');
  }
