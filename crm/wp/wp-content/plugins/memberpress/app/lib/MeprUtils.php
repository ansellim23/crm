...
/* PLUGGABLE FUNCTIONS AS TO NOT STEP ON OTHER PLUGINS' CODE */
  public static function get_currentuserinfo($id='') {
    self::include_pluggables('wp_get_current_user');
    ...
    else if(isset($current_user->ID) && $current_user->ID > 0) {
    ...
    public static function get_currentuserinfo_test($id) {
    self::include_pluggables('wp_get_current_user');
    $current_user = $id;

    if(isset($current_user) && $current_user > 0) {
      return new MeprUser($current_user);
    }
    else {
      return false;
    }
