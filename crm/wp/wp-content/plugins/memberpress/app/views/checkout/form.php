...
<!--         <input type="email" name="user_email" id="user_email<?php echo $unique_suffix; ?>" class="mepr-form-input" value="<?php echo (isset($user_id))?esc_attr(stripslashes($user_id)):''; ?>" required />
 -->
<div class="mp_wrapper mt-5 ">
  <form class="mepr-signup-form mepr-form" method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']).'#mepr_jump'; ?>" enctype="multipart/form-data" novalidate>
    ...
    <?php if((!MeprUtils::is_user_logged_in() ||
      ...
      <div class="mp-form-row mepr_first_name">
        ...
        <input type="text" readonly name="user_first_name" id="user_first_name<?php echo $unique_suffix; ?>" class="mepr-form-input" value="<?php echo esc_attr($first_name_value); ?>" <?php echo ($mepr_options->require_fname_lname)?'required':''; ?> />
      </div>
      <div class="mp-form-row mepr_last_name">
        ...
        <input type="text" readonly name="user_last_name" id="user_last_name<?php echo $unique_suffix; ?>" class="mepr-form-input" value="<?php echo esc_attr($last_name_value); ?>" <?php echo ($mepr_options->require_fname_lname)?'required':''; ?> />
      </div>
    ...
    <?php else: ?>
      ...
      <?php if(!$mepr_options->username_is_email): ?>
        <div class="mp-form-row mepr_username">
          ...
          <input type="text" readonly name="user_login" id="user_login<?php echo $unique_suffix; ?>" class="mepr-form-input" value="<?php echo (isset($user_login))?esc_attr(stripslashes($user_login)):''; ?>" required />
        </div>
      <?php endif; ?>
...
<!--         <a class="have-coupon-link" data-prdid="<?php echo $product->ID; ?>" href="">
    ...
    <div class="mp-form-submit">
      ...
      <input type="submit" class="mepr-submit" name="pay_now_button" value="<?php echo stripslashes($product->signup_button_text); ?>" />
...
// if(isset($_POST['pay_now_button'])){
     ...
     // echo "<pre>";
        ...
        // $wpdb->query($wpdb->prepare("DELETE FROM wpju_mepr_transactions WHERE product_id = '".$product->ID."' order by id desc limit 1,1"));
