	...
	jQuery( document ).ready( function() {
		...
		if ( $page_preloader.length > 0 ) {
			...
			$window.on( 'beforeunload', function(e) {
				if ( ( ! jQuery.browser || ! jQuery.browser.safari ) && ! $body.hasClass( 'ua_safari' ) ) {
				...
				setTimeout(function() {
					...
					// Shift page down on fixed rows height
						...
						if (obj.length > 0) {
							...
							if ( ! isNaN( off )
								&& ( ( trx_addons_fixed_rows_height() > 0 && off - _window_scroll_top < trx_addons_fixed_rows_height() + 60 )
			...
			var trx_addons_fix_column = function (e, cont) {
				...
				$fixed_columns.each(function() {
					...
					// Unfix on mobile layout (all columns are fullwidth)
						...
						if (col_height < row_height && _window_scroll_top + trx_addons_fixed_rows_height() > row_top) {
							...
							var col_big = col_height + 30 >= _window_height - trx_addons_fixed_rows_height(),
							...
							if (col_big) {
								...
								// If scrolling up
									...
									if (_window_scroll_top + trx_addons_fixed_rows_height() <= col_top) {
							...
							// If column height less then window height
								...
								if (_window_scroll_top + trx_addons_fixed_rows_height() >= row_top + row_height - col_height) {
									col_init.bottom = 0;
									col_init.position = 'absolute';
								} else {
									col_init.top = trx_addons_fixed_rows_height();
		...
		function trx_addons_stack_section(e, cont) {
			...
			var force = e.namespace == 'resize_trx_addons',
				wso = _window_scroll_top + trx_addons_fixed_rows_height();
