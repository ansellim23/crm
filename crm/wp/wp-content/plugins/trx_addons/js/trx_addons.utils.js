...
(function() {
	...
	*/
	...
	var _adminbar_update = function() {
		_adminbar_height = trx_addons_adminbar_height_calc();
	};
	$document.ready( _adminbar_update );
	$window.on( 'resize', _adminbar_update );
	...
	var _fixed_rows_update = function() {
		if ( $fixed_rows === false ) {
	...
	$document.ready( _fixed_rows_update );
	$document.on('action.ready_trx_addons action.sc_layouts_row_fixed_on action.sc_layouts_row_fixed_off', _fixed_rows_update );
	$window.on( 'resize', _fixed_rows_update );
...
//				&& ! $body.hasClass( 'header_position_over' )
	...
	window.trx_addons_browser_classes = function() {
		...
		classes['gecko'] = matchUserAgent('Gecko') && ! classes['chrome'];							// Gecko
		return classes;
	};
