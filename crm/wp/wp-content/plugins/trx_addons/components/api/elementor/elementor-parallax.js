( function() {
	...
	window.trx_addons_parallax = function( $target, init_type ) {
		var self          = this,
			...
			$targetLayers = $target,
			$targetBlocks = $target,
			wst           = _window_scroll_top + trx_addons_fixed_rows_height(),
			ww            = _window_width,
			wh            = _window_height - trx_addons_fixed_rows_height(),
			tl            = 0,
			tt            = 0,
			tw            = 0,
			th            = 0,
		...
		self.mouse_leave_handler = function( e ) {
			jQuery.each( mouse_list, function( index, block ) {
				...
				if ( scroller_init && scroller_init.css ) {
					x = x * 1 + ( scroller_init.css.x || 0 );
					y = y * 1 + ( scroller_init.css.y || 0 );
				}
