...
*/
			...
			'switcher' => array(
							'title' => __('Switch two blocks', 'trx_addons'),
							'layouts_sc' => array(
								'default' => esc_html__('Default', 'trx_addons')
							)
						),
...
if ( !function_exists( 'trx_addons_sc_load_responsive_styles' ) ) {
	...
	function trx_addons_sc_load_responsive_styles() {
		if (trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
			wp_enqueue_style( 'trx_addons-sc-responsive', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_SHORTCODES . 'shortcodes.responsive.css'), array(), null, trx_addons_media_for_load_css_responsive( 'sc', 'xl' ) );
		}
	}
}
...
if ( ! function_exists('trx_addons_sc_prepare_atts_before_output')) {
	...
	function trx_addons_sc_prepare_atts_before_output( $args, $query_args, $query, $sc = '' ) {
		if ( $args['orderby'] == 'rand'
			&& ! trx_addons_is_off( $args['pagination'] )
			&& $args['count'] > 0
